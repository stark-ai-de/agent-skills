...
- Do not commit `adapters/`. Generate OpenAI-native packages into disposable staging and archive them under `dist/openai/` with `pnpm run package:openai-plugin`; `pnpm run sync:openai-plugin` is a refuse-redirect.

...
The bundled gateway is loopback-only and applies a strict no-workspace-read/no-write/no-network Codex profile to every request, including SkillOpt candidate prompts. Keep it inside `skillopt-setup` while SkillOpt is its only workflow consumer; running this skill from Cursor or Claude Code does not change the `codex exec` backend or require a gateway variant. Extraction requires a second independent consumer plus fail-closed isolation. Remote publication additionally requires an infrastructure-owned OS/container boundary; workloads, route publication, NetworkPolicy, and secrets remain in the infrastructure source of truth.

...
- contains: Catalog candidates: 40
- contains: Selected + not selected: 40
...
- Put exactly one row for each of the 40 current ADRs with
...
- Reconcile the matrix totals: selected (`adopt` plus `adapt`) and not selected
  (`defer` plus `reject`) must sum to `Catalog candidates: 40`, with no duplicate

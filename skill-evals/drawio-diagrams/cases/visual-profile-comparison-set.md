...
Embed the supplied eval-orbit-mark.svg bytes in the Orbit Controller cell with aspect=fixed. Give Message Buffer a built-in native queue stencil and every other component a labelled built-in semantic icon. Keep component and boundary names, but add no explanatory prose, descriptions, metadata rows, or edge-label text.

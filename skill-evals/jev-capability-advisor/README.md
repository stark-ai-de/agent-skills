...
The new Native arm reuses exactly the frozen independent `next-skill-hidden32` inputs: **32 tasks × two repetitions**, with **48 skill and 16 NONE observations**. Requested model: **gpt-6-luna / medium**, Codex CLI 0.156.1. All **64/64** results are correct, with zero failed selections, timeouts, rejected model requests or tool executions. This adds **64** completed observations to the prior **15,100**, giving **15,164**; the existing 192 Jev/Hussi observations are reused without being counted again.

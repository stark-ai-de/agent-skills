...
| ------------------------------------------------------------- | ------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Known save destination; revise default and maximum, then save | 2             | One content checkpoint followed by the bounded revision/save card | All saved one requested spec only after approval; no repeated approval.                                 |
...
For `approval-once`, the candidate showed the complete proposed file before approval; the released arm showed a compact specification outline and expanded it during persistence. This improves reviewability in that trace, while also producing a longer answer.
...
Independent read-only Standards and Spec reviews at `6e336877` reported zero remaining actionable findings. Earlier family reviews found stale evaluation wording and a Claude rule-file classification edge case; both were fixed and rechecked before integration. Independent comparison of the public pilot receipt against all 21 local turns found one overbroad checkpoint label; it was corrected to distinguish the released outline from the candidate full draft. Hosted CI and the final PR head are reported on the pull request; this receipt is not release or publication evidence.

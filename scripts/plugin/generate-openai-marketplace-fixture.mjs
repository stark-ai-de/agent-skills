...
import { pluginArtifactPaths, pluginIdentity } from "../lib/release-descriptor.mjs";
...
try {
  ...
  const marketplace = renderMarketplace({
    name: paths.adapterFixtureName,
    displayName: paths.adapterFixtureDisplayName,
  ...
  const { errors } = validateMarketplaceDocument({
    ...
    expectedName: paths.adapterFixtureName,
    expectedDisplayName: paths.adapterFixtureDisplayName,
    pluginName: identity.name,
  });

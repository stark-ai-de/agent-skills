...
requirePattern(skillPath, skill, /version:\s*"0\.2\.0"/, "metadata.version must be 0.2.0");
requirePattern(
  skillPath,
  skill,
  /check selected analysis-tool updates once/i,
  "workflow must include the once-per-task selected-stack update check",
);
requirePattern(
  skillPath,
  skill,
  /every activation that will use installed analysis tools/i,
  "read-only analysis and planning must not silently skip selected-tool update checks",
);
requirePattern(
  skillPath,
  skill,
  /actively present one itemized checkpoint/i,
  "available updates must trigger an active itemized checkpoint",
);
requirePattern(
  skillPath,
  skill,
...
requirePattern(
  skillPath,
  skill,
  /Do not execute an unreviewed repository-wide rewrite; activate only to enforce the safety boundary/i,
  "unreviewed rewrite requests must still activate the skill's safety boundary",
);
...
requirePattern(
  updatePath,
  update,
  /at most once per tool per task/i,
  "stable lookup must be bounded to once per selected tool per task",
);
requirePattern(
  updatePath,
  update,
...
requirePattern(
  updatePath,
  update,
  /separate\/itemized for each install or update/i,
  "approval must be itemized per tool/action",
);
requirePattern(
  updatePath,
  update,
  /declining one item does not block diagnostics/i,
  "declining an update must preserve degraded operation",
);
requirePattern(
  updatePath,
  update,
  /does not authorize configuration repair or refresh/i,
  "binary/package and config authorization must stay separate",
);
requirePattern(
  updatePath,
  update,
...
requirePattern(
  extensionPath,
  extensions,
  /declines an optional extension or remote code execution.*metadata lookup is not tool execution/is,
  "declining optional execution must not suppress selected-core stable metadata checks",
);
...
if (behavioralManifest) {
  ...
  if (
    ...
    behavioralManifest.candidate?.skill_version !== "0.2.0"
  ) {
    fail(`${behavioralManifestPath}: candidate path/version does not match the public skill`);
  }
  if (
  ...
  const candidateHash = hashRuntimeCandidate(skillDir);
  if (behavioralManifest.candidate?.sha256 !== candidateHash) {
    fail(
      `${behavioralManifestPath}: captured candidate hash ${behavioralManifest.candidate?.sha256 || "<missing>"} does not match current runtime payload ${candidateHash}`,
    );
  }
  if (
...
requirePattern(rubricPath, rubric, /once per.*task/i, "rubric must grade update-check frequency");
requirePattern(rubricPath, rubric, /itemized/i, "rubric must grade per-tool update consent");
requirePattern(rubricPath, rubric, /legacy/i, "rubric must grade legacy capability fallback");
...
console.log(
  `Validated CodeGraph + ast-grep runtime contract, ${requiredEvalCases.length} scenario schemas, and ${capturedBehaviorAssertions} assertions across ${capturedBehaviorCases} captured behavioral cases.`,
);

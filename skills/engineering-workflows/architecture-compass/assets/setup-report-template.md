...
- Eligible catalog count (`Scope: target-repository`, `Adoptable: true`): `36`
- Matrix row count: `36`
- Selected count (`adopt` + `adapt`): `<number from completed matrix>`
- Not-selected count (`defer` + `reject`): `<number from completed matrix>`
- Total disposition count (`selected` + `not-selected`): `<number; must equal 36>`
- Count equality: `selected + not-selected = total = 36`: `pass | fail`

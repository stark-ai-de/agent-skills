...
- Eligible catalog count (`Scope: target-repository`, `Adoptable: true`): `37`
- Matrix row count: `37`
- Selected count (`adopt` + `adapt`): `<number from completed matrix>`
- Not-selected count (`defer` + `reject`): `<number from completed matrix>`
- Total disposition count (`selected` + `not-selected`): `<number; must equal 37>`
- Count equality: `selected + not-selected = total = 37`: `pass | fail`

...
- Eligible catalog count (`Scope: target-repository`, `Adoptable: true`): `40`
- Matrix row count: `40`
- Selected count (`adopt` + `adapt`): `<number from completed matrix>`
- Not-selected count (`defer` + `reject`): `<number from completed matrix>`
- Total disposition count (`selected` + `not-selected`): `<number; must equal 40>`
- Count equality: `selected + not-selected = total = 40`: `pass | fail`

...
8. Choose README and optional web-demo delivery from `references/github-readme-compatibility.md` and `references/asset-pipeline.md`. Export animated raster formats only when a detected tool can produce them and the result can be inspected. Never invent an artifact or successful export.
...
- If raster export or inspection capability is missing, keep the validated SVG and motion spec, report the unavailable capability, and create no placeholder.

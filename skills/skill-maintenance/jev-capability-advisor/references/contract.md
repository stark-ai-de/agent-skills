...
Only `use_when`, `keywords`, and parameter descriptions contribute positive lexical relevance. `avoid_when` never enters positive retrieval terms. Jev sees explicitly labeled, compact guidance within the existing card budget; long values are shortened. Known local source paths are removed before card truncation. Other provenance metadata stays local. With no guidance fields, the previous initial provider request format and initial instructions remain unchanged. Follow-up instructions separately check which independent tasks remain uncovered; a premature STOP is still incomplete, not success.

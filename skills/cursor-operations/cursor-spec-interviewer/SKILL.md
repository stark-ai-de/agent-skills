...
license: Apache-2.0
compatibility: Designed for Cursor Agent and Cursor Agent Skills. Works best with repo-local AGENTS.md, .cursor/rules, and project docs.
metadata:
  author: stark-ai-de
...
13. Produce a companion Cursor execution prompt using `assets/cursor-execution-prompt.md`.
14. Run a final self-check against `references/spec-rubric.md`.

## Cursor integration

- Use Cursor's structured question tool (AskQuestion) for option-style decisions such as mode selection, destination choice, or overwrite confirmation when it is available; otherwise ask conversationally.
- The interview can run in Cursor Plan Mode. Treat the saved spec file as the durable artifact that outlives Plan Mode's session plan, and still persist it per `references/artifact-destinations.md`.

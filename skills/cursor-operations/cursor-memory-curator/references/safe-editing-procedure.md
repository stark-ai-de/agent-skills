...
```bash
node scripts/backup-cursor-context.mjs --repo . --include PATH [--include PATH ...] [--backup-root PATH --backup-root-alias NAME]
```

Pass every file that may change as an exact repeatable `--include PATH`. Supplying any include selects exact mode and excludes project-rule, `AGENTS.md`, and memory-bank discovery; zero includes retain that legacy discovery and may use `--memory-bank`. Exact paths must be readable regular files, and every explicit memory-bank file or directory must exist and be readable. Any symlink path component, nested discovery symlink, unreadable directory, or traversal failure stops before backup-root creation. Before editing, verify that `backup-manifest.json` contains exactly one source entry for every intended changed file and that the copied size and SHA-256 match.
...
Report exact absolute backup and manifest paths only in non-persisted chat. A file-route receipt persists the script-reported `user-state/agent-memory-curator-backups/cursor/<identity-hash>/<backup-id>` or `external-root/<alias>/<backup-id>` storage locator, `<storage-locator>/backup-manifest.json`, and manifest-relative destinations; never copy absolute home, repository, source, or storage-root paths into a repository artifact.
...
```text
The Cursor memory-bank file format is unclear. I will not edit it directly. I will defer proposed replacements in the current curation result.
```

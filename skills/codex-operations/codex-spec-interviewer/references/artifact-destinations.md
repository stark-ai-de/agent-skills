...
When operating inside this `agent-skills` repository, existing saved specs use `docs/specs/do-not-publish/`; preserve that convention unless the user chooses a different public destination. In other repositories, follow the local convention first.
...
4. Add or update the ADR index when the repository has one.
...
Ask whether anything material is missing or wrong. Continue interviewing if the answer reveals a material gap. Otherwise save the spec by convention, and save any required ADR only after explicit confirmation, unless the user declined persistence.

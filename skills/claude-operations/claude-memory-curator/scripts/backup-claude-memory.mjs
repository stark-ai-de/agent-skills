...
function usage() {
  fs.writeSync(
    1,
    `Usage: backup-claude-memory.mjs [--repo PATH] [--claude-home PATH] [--memory-dir PATH] [--backup-root PATH --backup-root-alias NAME] [--include PATH ...]

Create a no-clobber backup of Claude Code memory and instruction files.

Options:
  --repo PATH         Use PATH instead of the current working directory.
  --claude-home PATH  Use PATH instead of ~/.claude.
  --memory-dir PATH   Discover this readable directory in legacy mode; it must exist.
...
files are copied. With no --include, legacy mode discovers the established Claude
context surfaces. Explicit discovery roots must exist and be readable. Legacy
discovery rejects symlinks, symlinked parents, and directory traversal errors.
Invalid settings JSON fails before backup-root creation. A present
autoMemoryDirectory that is not a non-empty string also fails before root creation.
By default, backups use a deterministic per-repository root below XDG_STATE_HOME (or
the user's portable local-state directory). The script rejects any default or
explicit backup root inside a Git worktree before copying.
It atomically creates .claude-context.backup.<timestamp>.<suffix>, never overwrites
a file, and writes ${manifestName} with size and SHA-256 evidence. Managed policy
files are not discovered. The script does not edit or delete context files.\n`,
  );
}
...
function parseArgs() {
  ...
  let claudeHomeWasExplicit = false;
...
function defaultBackupRoot(repo) {
  return path.join(stateHome(), "agent-memory-curator-backups", "claude", identityHash(repo));
}
...
function readableDirectoryIfPresent(value, label) {
  try {
    const stat = inspectPath(value, label, { allowMissing: true });
...
function walk(directory, predicate = () => true) {
  ...
  function visit(current) {
    ...
    for (const entry of entries) {
      ...
      const ignored =
        ignoredDirectories.has(entry.name) || entry.name.startsWith(".claude-context.backup.");
...
function readSettingsJson(file) {
  let data;
  try {
    data = JSON.parse(readStablePath(file, "Claude settings").toString("utf8"));
  } catch (error) {
    fail(`could not parse Claude settings JSON: ${file}: ${error.message}`, 1);
  }
  if (data === null || typeof data !== "object" || Array.isArray(data)) {
    fail(`Claude settings JSON must contain an object: ${file}`, 1);
  }
  return data;
}
...
function configuredAutoMemory(repo, claudeHome) {
  const settings = [
    ...
    .map((file) => ({ data: readSettingsJson(file), file }));

  for (const { data, file } of settings.reverse()) {
    if (!Object.hasOwn(data, "autoMemoryDirectory")) continue;
    if (typeof data.autoMemoryDirectory !== "string") {
      fail(`configured autoMemoryDirectory must be a string: ${file}`, 1);
    }
    const configured = data.autoMemoryDirectory.trim();
    if (!configured) {
      fail(`configured autoMemoryDirectory must be a non-empty path: ${file}`, 1);
    }
...
let memoryDir = null;
if (!exactMode) {
  if (claudeHomeWasExplicit) requireReadableDirectory(claudeHome, "--claude-home");
  if (explicitMemoryDir) {
    requireReadableDirectory(explicitMemoryDir, "--memory-dir");
    memoryDir = explicitMemoryDir;
  } else {
    const configuredMemoryDir = configuredAutoMemory(repo, claudeHome);
    if (configuredMemoryDir) {
      requireReadableDirectory(configuredMemoryDir, "configured auto-memory directory");
      memoryDir = configuredMemoryDir;
    } else {
      memoryDir = readableDirectoryIfPresent(
        derivedMemoryCandidate(repo, claudeHome),
        "derived auto-memory directory",
      );
    }
  }
}
...
const files = exactMode
  ? includePaths
  : [...collectProjectFiles(repo), ...collectUserFiles(claudeHome), ...walk(memoryDir)].filter(
      (file) => isLegacyRegularFile(file),
    );
if (files.length === 0) fail("No Claude context files found to back up.", 1);
...
const plan = buildPlan(files, (file, index) => {
  ...
  if (isWithin(claudeHome, file)) {
    return path.join("user-claude-home", path.relative(claudeHome, file));
  }
  if (memoryDir && isWithin(memoryDir, file)) {
    return path.join("auto-memory", path.relative(memoryDir, file));
  }
  fail(`discovered Claude context escaped its configured roots: ${file}`, 1);
});
...
const storageLocatorRoot = requestedBackupRoot
  ...
  : path.join("user-state", "agent-memory-curator-backups", "claude", repoIdentityHash);
const backup = createBackup(
  backupStorage,
  ".claude-context.backup",
  storageLocatorRoot,

...
```bash
node scripts/backup-claude-memory.mjs --repo .
```

Add `--claude-home PATH` or `--memory-dir PATH` for non-default locations. Record the backup path in the final answer.
...
```text
The Claude auto-memory file format is unclear. I will not edit it directly. I will write proposed replacements to a separate `.proposed.md` file instead.
```

Then write a sibling proposal file such as:

```text
debugging.md.proposed.md
```

...
| `DEC-002` | The committed repository marketplace points to the portable projection at `./plugins/stark-ai-developer`.                                                                                                                                                      | Codex CLI 0.147.0 or later can consume the portable Agent Plugins package directly, so the repository should exercise its canonical interoperability contract. | Successor ADR if the canonical marketplace must target a non-portable package. |
| `DEC-003` | Every bundled skill owns its reviewed `agents/openai.yaml` inside the canonical skill tree.                                                                                                                                                                    | The metadata is needed for standalone OpenAI skill distribution and must remain byte-identical across portable, OpenAI, and standalone projections.            | Successor ADR if adapters must overlay or replace skill-local metadata.        |
...
| `DEC-005` | Portable projection is generated and committed; the OpenAI adapter is generated into disposable staging and archived under `dist/openai/`; archives and adapter-marketplace fixtures are generated but not committed; sanitized release evidence is committed. | Reviewable portable projections catch drift, while native adapter trees and binary archives should remain reproducible outputs rather than editable sources.   | Architecture-owner approval.                                                   |
...
- [x] Host capability preflight, mutation boundaries, fallbacks, and stop conditions are documented and tested.

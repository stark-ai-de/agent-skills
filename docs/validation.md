...
The `Validate` workflow runs on pushes to `main`, pull requests, and manual dispatch. It runs `npm run validate`, `pnpm format:check`, `pnpm lint`, a pinned `npx --yes skills@1.5.22 add ./skills --list` discovery check in an isolated temporary copy, and `npm run smoke:install` (which uses the same pinned CLI unless `SKILLS_SMOKE_CLI` is explicitly configured). `npm run validate` includes actionlint for GitHub Actions workflows and the Astro site build. On pull requests, it also runs release validation when release intent is detected so partial package, changelog, or public skill version updates fail before merge. A single `trusted_main` output is computed from `push` to `main` or manual dispatch from `main` and reused for Pages configuration, artifact upload, receipt creation, and deployment; pull requests and manual non-main dispatches remain validation-only.
...
The required `Validate` workflow builds `site/` on every pull request through `npm run validate`, then seals the complete `site/dist` file-set digest before any later external CLI command and asserts that the sealed bytes remain unchanged before upload. The Pages artifact includes hidden files so its archive has the same file set as the seal. Every successful `push` to `main` and explicit manual dispatch from `main` then uploads and deploys the exact validated `site/dist` artifact through GitHub Pages Actions. The artifact names are derived by the artifact-producing `validate` job from its current attempt (`github-pages-<run-id>-<validation-job-attempt>` and `validation-receipt-<run-id>-<validation-job-attempt>`), and the receipt records both the workflow and validation-job attempt, candidate fingerprint/file count before the gates, from smoke-copy, and after the gates, the CLI version, normalized smoke override state, workflow/run identity, SHA/event/branch/version, site digest, and artifact names/IDs. Release readiness resolves a successful main-push run by exact SHA, discovers the successful `validate` job attempt through the Actions API, verifies REST metadata, downloads the named artifacts by explicit run ID, recomputes both the candidate and Pages digests, and fails closed on missing, expired, malformed, or mismatched proof. Production Pages deployments share one concurrency group and verify that `refs/heads/main` still equals the run SHA immediately before deployment, so stale manual runs cannot roll Pages backward. The publication job repeats the proof and freshness checks at its own boundary. The pnpm cache is lockfile-bound dependency state and is never validation proof.

...
ARCHITECTURE_FIXTURE_COPY_MODE=copy npm run validate:architecture-compass
```
...
Manifest schema v2 separates affected-selection paths from complete execution inputs. The resolver computes canonical content-addressed task keys from repository, gate and engine contracts; path, type, mode, size, and content witnesses; expanded commands; dependency profiles; prerequisite keys; sanitized environment; exact tools and platform; logical Git inputs; evidence; and restorable outputs. Commit SHA, run identity, timestamps, and duration are excluded. Missing, ambiguous, or unverifiable identity forces a miss. `validation_reuse` accepts `auto`, `off`, or `verify`: `auto` reuses verified hits, `off` executes every selected gate, and `verify` re-executes would-be hits and requires identical semantic evidence and output digests.

...
This gate verifies the complete 55-record Short/Long/Guide inventory, shared metadata and navigation, scope/adoption rules, accepted decision locks, catalog routing, the exhaustive repo-only decision-lineage disposition manifest, the separately validated internal implementation namespace whose shipped records use Accepted or Superseded status, immutable decision locks, reciprocal successor metadata, and exclusion from public catalog/adoption routing, the five public workflows and intent-bound selector, derived assets, retired legacy references, and required lifecycle/eval cases. The install smoke requires the exact AC-ADR-001 through AC-ADR-055 inventory plus the seven internal reference files, checks each shipped internal record has an allowed historical status, compares each installed payload with the clean-copy source payload, and rejects leaked copies of the repo-only lineage and historical-reference evidence by filename or exact content hash.
...
pnpm run release:validate -- --version "$NEXT_VERSION" --base-ref origin/main
...
- The advisory runtime matrix classifies every current execution boundary, keeps unknown signals non-blocking, aligns winners with package scripts and CI, and requires evidence plus a revisit trigger for each fallback.
...
- Architecture Compass validation checks its 55 routed ADR triplets, five-workflow dispatcher, preserved AC-ADR-002 to AC-ADR-026 to AC-ADR-043 to AC-ADR-045 to AC-ADR-048 workflow history, AC-ADR-042 to AC-ADR-047 to AC-ADR-049 validation-policy succession, independent AC-ADR-046 evidence ranking, AC-ADR-050 semantic receipt markers, AC-ADR-051 public/internal namespace routing, AC-ADR-052 host-neutral persistence surfaces, AC-ADR-053 capability-aware presentation profiles, AC-ADR-054 external agent-worktree isolation, AC-ADR-055 pnpm-owned persistent dependencies with fail-closed Bun-first execution, complete decision-lineage dispositions, byte-locked legacy-reference coverage outside the runtime payload, AC-ADR-004-aligned receipt assets, derived non-normative assets, locked and reciprocally superseded internal-triplet integrity, and maintainer eval inventory.
...
- Script syntax validation uses Node.js for syntax-only checking, validates skill shell scripts with Bash, then exercises focused install-contract and release-intent fixtures under the Bun parent runtime.

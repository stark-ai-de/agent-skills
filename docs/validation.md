...
This gate verifies the complete 64-record Short/Long/Guide inventory (192 variant files), shared metadata and navigation, scope/adoption rules, accepted decision locks, catalog routing, the exhaustive repo-only decision-lineage disposition manifest, the separately validated internal implementation namespace whose shipped records use Accepted or Superseded status, immutable decision locks, reciprocal successor metadata, and exclusion from public catalog/adoption routing, the five public workflows and intent-bound selector, derived assets, retired legacy references, and required lifecycle/eval cases. The install smoke requires the exact AC-ADR-001 through AC-ADR-064 inventory, followed by the seven internal reference files, checks each shipped internal record has an allowed historical status, compares each installed payload with the clean-copy source payload, and rejects leaked copies of the repo-only lineage and historical-reference evidence by filename or exact content hash.
...
- Interviewer validation checks the three portable runtime variants, shared lifecycle consistency, template records, local reference resolution and the 22-scenario approval inventory. These are structural assertions; the separately recorded Astra CLI pilot is bounded live conversation evidence, not native UI or cross-client qualification.
- Memory-curator validation checks the shared eight-route ordering, intent-matched recommendation, routing defaults, persistence records, Plan lifecycle, cleanup boundaries, exact-include no-clobber backups, and source-to-copy manifest integrity across Claude, Codex, and Cursor.
- Architecture Compass validation checks its 64 public ADR triplets (001–064), five-workflow dispatcher, preserved AC-ADR-002 to AC-ADR-026 to AC-ADR-043 to AC-ADR-045 to AC-ADR-048 to AC-ADR-064 workflow history, AC-ADR-042 to AC-ADR-047 to AC-ADR-049 validation-policy succession, independent AC-ADR-046 evidence ranking, AC-ADR-050 semantic receipt markers, AC-ADR-051 public/internal namespace routing, AC-ADR-052 host-neutral persistence surfaces, AC-ADR-053 capability-aware presentation profiles, AC-ADR-054 external agent-worktree isolation, AC-ADR-055 generated-release separation plus the AC-ADR-056 and AC-ADR-057 recovery refinements, AC-ADR-058 pnpm ownership with Bun-first verified execution, AC-ADR-059 through AC-ADR-062 measurable testing contracts, AC-ADR-063 shadcn lint adoption, AC-ADR-064 capability-aware planning, complete decision-lineage dispositions, byte-locked legacy-reference coverage outside the runtime payload, AC-ADR-004-aligned receipt assets, derived non-normative assets, locked and reciprocally superseded internal-triplet integrity, and maintainer eval inventory.
...
See the [GitHub Actions workflow list](publishing.md#github-actions-workflows) for
the displayed names, responsibilities, and triggers.
...
`pnpm run verify:openai-directory` is a manual post-publication diagnostic for
directory-document identity (`DIR-001`) and public category-catalog membership
(`DIR-002`). It remains strict and does not package an OpenAI zip. The scheduled
workflow and composite action were removed because the unofficial catalog could
not reliably verify the listing. See [manual directory observation](publishing.md#manual-directory-observation)
for the dated evidence and limitations. Deterministic hosted `Validate` and the
local aggregate do not fetch the live directory.

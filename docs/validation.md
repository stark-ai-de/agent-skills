...
The required `Validate` workflow already builds `site/` on pull requests through `npm run validate`. Under [ADR-0043](adrs/0043-deploy-validated-main-artifacts.short.md) ([Long, canonical](adrs/0043-deploy-validated-main-artifacts.long.md) · [Guide](adrs/0043-deploy-validated-main-artifacts.guide.md)), the same successful `main` push or explicit manual-main run uploads the static `site/dist` artifact and a SHA-bound validation receipt, retains both for 30 days, and deploys that artifact in a dependent job. Non-main manual validation remains validation-only; there is no separate Pages build workflow.

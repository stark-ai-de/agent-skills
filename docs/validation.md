...
The required `Validate` workflow builds `site/` on every pull request through `npm run validate`. Every successful `push` to `main` and explicit manual dispatch from `main` then uploads and deploys the exact validated `site/dist` artifact through GitHub Pages Actions. The artifact names are attempt-scoped (`github-pages-<run-id>-<run-attempt>` and `validation-receipt-<run-id>-<run-attempt>`), and the receipt records the candidate fingerprint/file count before the gates, from smoke-copy, and after the gates, the CLI version, normalized smoke override state, workflow/run identity, SHA/event/branch/version, site digest, and artifact names/IDs. Release readiness resolves a successful main-push run and attempt by exact SHA, verifies REST metadata, downloads the named artifacts by explicit run ID, recomputes the candidate fingerprint, and fails closed on missing, expired, malformed, or mismatched proof. The pnpm cache is lockfile-bound dependency state and is never validation proof.

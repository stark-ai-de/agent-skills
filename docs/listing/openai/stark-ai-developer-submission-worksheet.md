...
- Release notes: Adds AC-ADR-055 in Architecture Compass 0.6.6 for pnpm-owned persistent dependencies, fail-closed Bun-first execution, collision-safe script entrypoints, explicit transient CLI trust boundaries, Vite/Vitest defaults, and optimized Bun server artifacts.

...
- AC-ADR-058 provides a useful Bun-first repository-tooling candidate, while AC-ADR-014 requires evidence for the actual executable or deployable.
- Separating package ownership and default candidacy here from final per-boundary selection in ADR-0054 preserves both responsibilities.
...
- Chosen: pnpm ownership plus a Bun-first candidate, coordinated with ADR-0054's advisory evidence matrix.
...
- Risk: Blindly forcing Bun could introduce subtle failures; ADR-0054 and mandatory command checks contain that risk.

...
1. Local installation remains isolated, exactly pinned and loopback-only. Remote mode does not change this local exposure boundary.
2. Remote plans inspect an explicitly selected management endpoint and preserve path prefixes and the deployed API version.
3. Create or reuse API-managed routes; give a patch and instructions for declarative routes instead of overwriting them.
4. Read back writes and verify inference separately. Reconcile ambiguous outcomes before retrying.
5. Model discovery and client-specific tool/result flows determine compatibility; unsupported required semantics block stronger claims.

## Options

- Chosen: Use official LiteLLM locally or an existing remote gateway, preserve configuration ownership, and verify provider, gateway, and clients separately.
